...
declaration_list : declaration_list COMMA id_name
		  {
			...
			$$ = new symbol_info(varlist,"decl_list");
			$$->set_ast_node(nullptr);
...
factor	: variable  // factor can be void
	...
	| variable INCOP 
	{
		...
		VarNode* lhsVarNode = new VarNode($1->getname(), $1->getvartype());
		VarNode* rhsVarNode = new VarNode($1->getname(), $1->getvartype());
		ConstNode* oneNode = new ConstNode("1", "int");
		BinaryOpNode* addNode = new BinaryOpNode("+", rhsVarNode, oneNode, $1->getvartype());
		AssignNode* assignNode = new AssignNode(lhsVarNode, addNode, $1->getvartype());
		$$->set_ast_node(assignNode);
	}
	| variable DECOP
	{
		...
		VarNode* rhsVarNode = new VarNode($1->getname(), $1->getvartype());
		ConstNode* oneNode = new ConstNode("1", "int");
		BinaryOpNode* subNode = new BinaryOpNode("-", rhsVarNode, oneNode, $1->getvartype());
		AssignNode* assignNode = new AssignNode(lhsVarNode, subNode, $1->getvartype());
		$$->set_ast_node(assignNode);
	}
	;
