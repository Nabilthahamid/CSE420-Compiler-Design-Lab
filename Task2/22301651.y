...
func_definition : type_specifier ID LPAREN { current_param_list.clear(); } parameter_list RPAREN 
		{
	...
	| type_specifier ID LPAREN { current_param_list.clear(); } RPAREN 
		{
			$2->set_symbol_kind("function");
			$2->set_data_type($1->get_name());
			$2->set_param_details({});
...
declaration_list : declaration_list COMMA ID
		  {
			...
			$3->set_data_type(current_var_type);
			sym_table->insert($3, outlog);
 		  }
 		  ...
 		  | ID
 		  {
			...
			$1->set_data_type(current_var_type);
			sym_table->insert($1, outlog);
 		  }
